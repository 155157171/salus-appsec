...
export async function applyFix(filePath, oldCode, newCode) {
    ...
    if (!content.includes(oldCode)) {
        throw new Error(`Trecho de código antigo não encontrado em "${filePath}". ` +
    ...
    if (content === patched) {
        throw new Error(`Substituição não alterou o arquivo "${filePath}". ` +
    ...
    console.log(`[Salus] Backup salvo em: ${backupPath} ` +
